...
//yangzhe add for af test on 2013.1.5
		else if(write_arr[i].reg_write_type == MSM_ACTUATOR_WRITE_HW_DAMP)
		{
			buf[0] = (hw_dword & 0xff00)>>8;//MSB
			buf[1] = hw_dword & 0xff;//LSB
			length = 2;
			printk("[yangzhe]%s: position is 0x%x, MSB0x%x, LSB:0x%x\r\n", __func__, value, buf[0], buf[1]);
			rc = msm_camera_i2c_txdata(&a_ctrl->i2c_client, buf, length);
			if (rc < 0) {
				pr_err("[yangzhe]%s: i2c write error:%d\n",
					__func__, rc);
				return rc;
			}
			break;
			...
			printk("%s:MSM_ACTUATOR_WRITE_DAC_AD5823",__func__);
			value = (next_lens_position <<
				write_arr[i].data_shift) |
				((hw_dword & write_arr[i].hw_mask) <<
				write_arr[i].hw_shift);
		...
		//wang_gj add for BU6424 begin
		else if (write_arr[i].reg_write_type == MSM_ACTUATOR_WRITE_DAC_BU6424) {
			value = (next_lens_position <<
				write_arr[i].data_shift) |
				((hw_dword & write_arr[i].hw_mask) <<
				write_arr[i].hw_shift);
		...
		//wang_gj add for BU6424 end
		//wang_gj add for DW9174 begin
		else if (write_arr[i].reg_write_type == MSM_ACTUATOR_WRITE_DAC_DW9174) {
			value = (next_lens_position <<
				write_arr[i].data_shift) |
				((hw_dword & write_arr[i].hw_mask) <<
				write_arr[i].hw_shift);
			...
			buf[0] = write_arr[i].reg_addr;
			buf[1] = (value & 0x3f00)>>8;//MSB
			buf[0] = buf[0] |buf[1]; 
			buf[2] = value & 0xff;//LSB
			buf[1] = buf[2]; 
			length = 2;
			CDBG("%s: DW9174 position is 0x%x, MSB0x%x, LSB:0x%x\r\n", __func__, value, buf[1], buf[2]);
			rc = msm_camera_i2c_txdata(&a_ctrl->i2c_client, buf, length);
			if (rc < 0) {
				pr_err("%s: i2c write error:%d\n",
					__func__, rc);
		...
		//wang_gj add for DW9174 end
		//wuchx added for a3907
		else if (write_arr[i].reg_write_type == MSM_ACTUATOR_WRITE_DAC_A3907) {
			printk("%s:MSM_ACTUATOR_WRITE_DAC_A3907",__func__);
			#if 0
			value = (next_lens_position <<
				write_arr[i].data_shift) |
				((hw_dword & write_arr[i].hw_mask) <<
			...
			buf[1] = (value  << 4) & 0xf0;
			length = 2;
			CDBG("%s: next_lens_position is %d,position is 0x%x, MSB0x%x, LSB:0x%x\r\n", 
					__func__, next_lens_position,value, buf[1], buf[2]);
			rc = msm_camera_i2c_txdata(&a_ctrl->i2c_client, buf, length);
			if (rc < 0) {
				pr_err("%s: i2c write error:%d\n",
					__func__, rc);
