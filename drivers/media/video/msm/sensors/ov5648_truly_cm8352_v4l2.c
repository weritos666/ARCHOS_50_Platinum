...
//wuchx 2013-1-14 added for get module id 
static int32_t ov5648_module_id = CAM_MODULE_TRULY;
...
static struct msm_camera_i2c_reg_conf ov5648_truly_cm8352_pip_snap_settings_master[] = {
	...
	{0x3837, 0x37},
	{0x383a, 0x01},
#ifdef TYQ_CAMERA_OV7695_RAW_FLIP
	{0x383b, 0xe0},//	; 80 PIP Location
#else
	...
	{0x4004, 0x04},//	; black line number
#if 1
	{0x4005, 0x1a}, //BLC always update   yangzhe open on 2013.01.06
#else
...
int32_t pip_ov5648_ctrl(int cmd, struct msm_camera_pip_ctl *pip_ctl)
{
	...
	switch(cmd)
	{
		...
		case PIP_CTL_STREAM_ON:
				...
				client.addr = ov5648_i2c_address;
				sensor_i2c_client.client = &client;
				msm_camera_i2c_write(&sensor_i2c_client, 0x301c, 0xf0,
					MSM_CAMERA_I2C_BYTE_DATA);
		...
		case PIP_CTL_STREAM_OFF:
				...
				msm_camera_i2c_write(&sensor_i2c_client, 0x301a, 0xf1,
					MSM_CAMERA_I2C_BYTE_DATA);
				msm_camera_i2c_write(&sensor_i2c_client, 0x301c, 0xf4,
					MSM_CAMERA_I2C_BYTE_DATA);
				break;
...
static struct msm_camera_i2c_reg_conf ov5648_truly_cm8352_pip_start_settings[] = {
	{0x301c, 0xf0},  /* streaming on */
	{0x301a, 0xf0},  
};

static struct msm_camera_i2c_reg_conf ov5648_truly_cm8352_pip_stop_settings[] = {
	{0x301a, 0xf1},  /* streaming off*/
	{0x301c, 0xf4},  
};
...
int32_t OV5648_write_i2c(uint16_t addr, uint16_t data)
{
...
uint16_t OV5648_read_i2c(uint16_t addr)
{
    uint16_t temp;
...
int check_otp(int index)
{
	...
	{
		// read otp --Bank 0
		OV5648_write_i2c(0x3d84, 0xc0);
		OV5648_write_i2c(0x3d85, 0x00);
		OV5648_write_i2c(0x3d86, 0x0f);
		OV5648_write_i2c(0x3d81, 0x01);
		msleep(5);
		flag = OV5648_read_i2c(0x3d05);
		rg = OV5648_read_i2c(0x3d07);
		bg = OV5648_read_i2c(0x3d08);
	}
	else if (index == 2)
	{
		// read otp --Bank 0
		...
		OV5648_write_i2c(0x3d81, 0x01);
		msleep(5);
		flag = OV5648_read_i2c(0x3d0e);
		// read otp --Bank 1
		OV5648_write_i2c(0x3d84, 0xc0);
		OV5648_write_i2c(0x3d85, 0x10);
		OV5648_write_i2c(0x3d86, 0x1f);
		OV5648_write_i2c(0x3d81, 0x01);
		msleep(5);
		rg = OV5648_read_i2c(0x3d00);
		bg = OV5648_read_i2c(0x3d01);
	}
	else if (index == 3)
	{
		// read otp --Bank 1
		...
		OV5648_write_i2c(0x3d81, 0x01);
		msleep(5);
		flag = OV5648_read_i2c(0x3d07);
		rg = OV5648_read_i2c(0x3d09);
		bg = OV5648_read_i2c(0x3d0a);
	}
	flag = flag & 0x80;
	// clear otp buffer
	for (i=0;i<16;i++) {
		OV5648_write_i2c(0x3d00 + i, 0x00);
	}
	if (flag) {
		return 1;
	}
...
int read_otp(int index,  struct otp_struct *otp_ptr)
{
	...
	if (index == 1)
	{
		...
		OV5648_write_i2c(0x3d81, 0x01);
		msleep(5);
		(*otp_ptr).module_integrator_id = (OV5648_read_i2c(0x3d05) & 0x7f);
		(*otp_ptr).lens_id = OV5648_read_i2c(0x3d06);
		temp = OV5648_read_i2c(0x3d0b);
		(*otp_ptr).rg_ratio = (OV5648_read_i2c(0x3d07)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (OV5648_read_i2c(0x3d08)<<2) + ((temp>>4) & 0x03);
		(*otp_ptr).light_rg = (OV5648_read_i2c(0x3d0c)<<2) + ((temp>>2) & 0x03);
		(*otp_ptr).light_bg = (OV5648_read_i2c(0x3d0d)<<2) + (temp & 0x03);
		(*otp_ptr).user_data[0] = OV5648_read_i2c(0x3d09);
		(*otp_ptr).user_data[1] = OV5648_read_i2c(0x3d0a);
	}
	else if (index == 2)
	{
		// read otp --Bank 0
		...
		OV5648_write_i2c(0x3d81, 0x01);
		msleep(5);
		(*otp_ptr).module_integrator_id = (OV5648_read_i2c(0x3d0e) & 0x7f);
		(*otp_ptr).lens_id = OV5648_read_i2c(0x3d0f);
		// read otp --Bank 1
		OV5648_write_i2c(0x3d84, 0xc0);
		OV5648_write_i2c(0x3d85, 0x10);
		OV5648_write_i2c(0x3d86, 0x1f);
		OV5648_write_i2c(0x3d81, 0x01);
		msleep(5);
		temp = OV5648_read_i2c(0x3d04);
		(*otp_ptr).rg_ratio = (OV5648_read_i2c(0x3d00)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (OV5648_read_i2c(0x3d01)<<2) +( (temp>>4) & 0x03);
		(*otp_ptr).light_rg = (OV5648_read_i2c(0x3d05)<<2) + ((temp>>2) & 0x03);
		(*otp_ptr).light_bg = (OV5648_read_i2c(0x3d06)<<2) + (temp & 0x03);
		(*otp_ptr).user_data[0] = OV5648_read_i2c(0x3d02);
		(*otp_ptr).user_data[1] = OV5648_read_i2c(0x3d03);
	}
	else if (index == 3)
	{
		// read otp --Bank 1
		...
		(*otp_ptr).module_integrator_id = (OV5648_read_i2c(0x3d07) & 0x7f);
		(*otp_ptr).lens_id = OV5648_read_i2c(0x3d08);
		temp = OV5648_read_i2c(0x3d0d);
		(*otp_ptr).rg_ratio = (OV5648_read_i2c(0x3d09)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (OV5648_read_i2c(0x3d0a)<<2) +( (temp>>4) & 0x03);
		(*otp_ptr).light_rg = (OV5648_read_i2c(0x3d0e)<<2) + ((temp>>2) & 0x03);
		(*otp_ptr).light_bg = (OV5648_read_i2c(0x3d0f)<<2) +(temp & 0x03);
		(*otp_ptr).user_data[0] = OV5648_read_i2c(0x3d0b);
		(*otp_ptr).user_data[1] = OV5648_read_i2c(0x3d0c);
	}
	//wuchx add for module id/info begin 
	ov5648_module_id = otp_ptr->module_integrator_id;
     CDBG("%s-otp code version = 2.51\r\n", __func__);
     CDBG("==========OV5648 OTP INFO==========\r\n"); 
     CDBG("%s-otp group index     = 0x%x\r\n", __func__, index);	 
	...
	//wuchx add for module id/info end
...
{
	if (R_gain>0x400) {
		OV5648_write_i2c(0x5186, R_gain>>8);
		OV5648_write_i2c(0x5187, R_gain & 0x00ff);
	}
	if (G_gain>0x400) {
		OV5648_write_i2c(0x5188, G_gain>>8);
		OV5648_write_i2c(0x5189, G_gain & 0x00ff);
	}
	if (B_gain>0x400) {
		OV5648_write_i2c(0x518a, B_gain>>8);
		OV5648_write_i2c(0x518b, B_gain & 0x00ff);
	}
	return 0;
}
...
int update_otp(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(current_otp.light_bg==0) {
     ...
     //wuchx 2013-1-17 added for moudule detect begin
     if(CAM_MODULE_TRULY == ov5648_module_id){
	 	//truly
		RG_Ratio_Typical = 0x125;
		BG_Ratio_Typical = 0x185;
     }else if(CAM_MODULE_SUNNY == ov5648_module_id){
     		#ifdef TYQ_CAMRTA_ACT_11_SUPPORT
     		//sunny q5v22e(cost down)
     		RG_Ratio_Typical = 0x167;
		BG_Ratio_Typical = 0x164;
		#else
     		//sunny q5v22b
     		RG_Ratio_Typical = 0x125;
		BG_Ratio_Typical = 0x119;
		#endif
     }else{
     		//o-film
  		RG_Ratio_Typical = 0x171;
		BG_Ratio_Typical = 0x168;
     }
    CDBG(KERN_ERR "RG_Ratio_Typical = 0x%x,BG_Ratio_Typical = 0x%x\n",
		RG_Ratio_Typical,BG_Ratio_Typical);
     //wuchx 2013-1-17 added for moudule detect end	
...
//wuchx 2013-3-11 added for reading cunstomer_id at bring-up
int ov5648_truly_cm8352_read_customer_id(struct msm_sensor_ctrl_t *s_ctrl)
{
     struct otp_struct current_otp;
     int i;
     int otp_index;
...
{
	return sprintf(buf, "%d", ov5648_new_mode);
}
static ssize_t sensor_store_PIP_mode(struct device *dev,
			struct device_attribute *attr,const char *buf,size_t count)
{
	unsigned long value = simple_strtoul(buf, NULL, 10);
	CDBG("Enter %s:PIP_mode = %lu\n",__func__,value);
	ov5648_new_mode  = value > 0 ? CAM_MODE_PIP : CAM_MODE_NORMAL;  
	CDBG("Exit  %s:PIP_mode = %d\n",__func__,ov5648_new_mode);
	return count;	
}
		...
		const struct i2c_device_id *id)
{
	...
	if (client->dev.platform_data == NULL) {
 	...
 	//wuchx 2013-3-11 added for reading cunstomer_id at bring-up
	//o-film : DW9714 : MSM_ACTUATOR_MAIN_CAM_9
 	//sunny/truly AD5820:MSM_ACTUATOR_MAIN_CAM_7
 	//sunny cost down: A3907:MSM_ACTUATOR_MAIN_CAM_11
 	if(info->actuator_info != NULL){
 		CDBG("%s: Re-config act name module_id = %d\n", __func__,ov5648_module_id);
 		if(CAM_MODULE_OFILM == ov5648_module_id){
			info->actuator_info->cam_name = MSM_ACTUATOR_MAIN_CAM_9;
 		}else if(CAM_MODULE_SUNNY == ov5648_module_id){
 			#ifdef TYQ_CAMRTA_ACT_11_SUPPORT
 			info->actuator_info->cam_name = MSM_ACTUATOR_MAIN_CAM_11;
 			#else
 			info->actuator_info->cam_name = MSM_ACTUATOR_MAIN_CAM_7;
 			#endif
 		}else{
 			info->actuator_info->cam_name = MSM_ACTUATOR_MAIN_CAM_7;
 		}
	...
	CDBG("%s: Create SYSFS Enterance  for PIP mode\n", __func__);
			...
			int update_type, int res)
{
	...
	else if(CAM_MODE_PIP == ov5648_working_mode)
	{
		...
		if (update_type == MSM_SENSOR_REG_INIT) {
			...
			/*ov7695 sw standby*/
			pip_ov7695_ctrl(PIP_CTL_STANDBY_SW, &pip_ctl);	
			/*OV7695 pip mode settings*/
			pip_ctl.write_ctl = res;
			pip_ov7695_ctrl(PIP_CRL_WRITE_SETTINGS, &pip_ctl);
			//msleep(60);
			/*OV5648 pip mode settings*/
			msm_sensor_write_conf_array(
				s_ctrl->sensor_i2c_client,
				s_ctrl->msm_sensor_reg->mode_settings, res);
			msleep(30);
			...
			/*Delay 1 frame in PIP mode*/
			if(res == MSM_SENSOR_RES_FULL){
				msleep(100);
			}else{
				msleep(50);
			}
			/*ov5648 sw standby exit*/
			pip_ov5648_ctrl(PIP_CTL_STANDBY_EXIT, &pip_ctl);	
			/*ov7695 sw standby exit*/
			pip_ov7695_ctrl(PIP_CTL_STANDBY_EXIT, &pip_ctl);	
...
//wuchx 2013-1-14 added for get module id 
int32_t ov5648_turly_cm8352_get_module_id(struct msm_sensor_ctrl_t *s_ctrl,
	int32_t * module_id){

	CDBG("%s IN, module id is %d\r\n", __func__, ov5648_module_id);
	*module_id = ov5648_module_id;
	CDBG("%s, X\n",__func__);
	return 0;	
}
...
static struct msm_sensor_fn_t ov5648_truly_cm8352_func_tbl = {
	...
	//wuchx 2013-1-14 added for get module id 
	.sensor_get_module_id = ov5648_turly_cm8352_get_module_id,
	//wuchx 2013-3-11 added for reading cunstomer_id at bring-up
	.sensor_customize_func = ov5648_truly_cm8352_read_customer_id,
};
		...
		struct sensor_output_info_t *sensor_output_info)
{
	...
	if(CAM_MODE_NORMAL == ov5648_working_mode)
	{
		...
		ov5648_truly_cm8352_regs.start_stream_conf = ov5648_truly_cm8352_start_settings;
		ov5648_truly_cm8352_regs.start_stream_conf_size = ARRAY_SIZE(ov5648_truly_cm8352_start_settings);
		ov5648_truly_cm8352_regs.stop_stream_conf = ov5648_truly_cm8352_stop_settings;
		ov5648_truly_cm8352_regs.stop_stream_conf_size = ARRAY_SIZE(ov5648_truly_cm8352_stop_settings);
	}
	else if(CAM_MODE_PIP == ov5648_working_mode)
	{
		...
		ov5648_truly_cm8352_regs.start_stream_conf = ov5648_truly_cm8352_pip_start_settings;
		ov5648_truly_cm8352_regs.start_stream_conf_size = ARRAY_SIZE(ov5648_truly_cm8352_pip_start_settings);
		ov5648_truly_cm8352_regs.stop_stream_conf = ov5648_truly_cm8352_pip_stop_settings;
		ov5648_truly_cm8352_regs.stop_stream_conf_size = ARRAY_SIZE(ov5648_truly_cm8352_pip_stop_settings);
	}
