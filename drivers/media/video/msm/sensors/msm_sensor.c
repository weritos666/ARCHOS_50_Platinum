...
int32_t msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	cdata.cfgtype);
		...
		case CFG_SENSOR_GET_MODULE_ID:
			if (s_ctrl->func_tbl->
			sensor_get_module_id == NULL) {
				rc = 0;
				break;
			}
			rc = s_ctrl->func_tbl->
				sensor_get_module_id(
				s_ctrl,
				&cdata.cfg.module_id);
	...
	const struct i2c_device_id *id)
{
	...
	//wuchx 2013-3-11 added for customize function
	if(s_ctrl->func_tbl->sensor_customize_func)
		rc = s_ctrl->func_tbl->sensor_customize_func(s_ctrl);
