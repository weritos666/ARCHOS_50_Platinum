...
if (pstrid_ssd != NULL) { //is ssd2080m run
   ...
   if (tyq_get_lcd_id()==1)  // truly lcd only 
	{
      //make the lane into ulps
      MIPI_OUTP(MIPI_DSI_BASE+0xa8, 0x13);
      //0x1 for one lane
      //0x3 for two lane,
      //0x10 for clk lane
      mdelay(1);
...
if (tyq_get_lcd_id()==1)  // truly lcd
{
    ...
    /*tydrv wuxh  add for truly hd lcd */	
#if defined(TYQ_HX8392_MIPI_HD_SUPPORT)
#if defined(TYQ_MIPI_LCD_DRV_COMPAT_SUPPORT) && defined(TYQ_SSD2080M_MIPI_HD_SUPPORT)
if (pstrid_ssd == NULL) { //is not ssd2080m run
	MIPI_OUTP(MIPI_DSI_BASE+0xa8, 0x1f);
	mdelay(1);
}
#else
	MIPI_OUTP(MIPI_DSI_BASE+0xa8, 0x1f);
...
pstrid_ssd = strstr(get_aboot_cmd, "ssd2080m");
if (pstrid_ssd == NULL) { //is not ssd2080m run
	 MIPI_OUTP(MIPI_DSI_BASE+0xa8, 0x1f00); //lane exit ulps,
      //0x100 for one
      //0x300 for two lane
      //0x1000 for clk lane
      mdelay(1);
...
#ifdef TYQ_TRULY_SLEEP_IN_RESET_HIGH
 if (tyq_get_lcd_id()==1)  // truly lcd only 
   {   //make the lane out of ulps
      MIPI_OUTP(MIPI_DSI_BASE+0xa8, 0x1300); //lane exit ulps,
      //0x100 for one
      //0x300 for two lane
      //0x1000 for clk lane
      mdelay(1);
