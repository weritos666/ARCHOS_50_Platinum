...
/*TYRD wuxh add begin for bug CQWB00000946 on 20121129*/
static int unset_bl_level, bl_updated;
static int bl_level_old;
/*TYRD wuxh add end for bug CQWB00000946 on 20121129*/
...
/*TYRD wuxh add for backlight control on 20121215*/
...
#endif
/*TYRD wuxh move following code to up line for bug CQWB00000946 on 20121129*/
//static int unset_bl_level, bl_updated;
//static int bl_level_old;
static int mdp_bl_scale_config(struct msm_fb_data_type *mfd,
						struct mdp_bl_scale_data *data)
...
void msm_fb_set_backlight(struct msm_fb_data_type *mfd, __u32 bkl_lvl)
{
	struct msm_fb_panel_data *pdata;
	__u32 temp = bkl_lvl;
	/*TYRD wuxh add for bl debug on 20130222 */
	static __u32 pre_bkl_lvl=0;

	if(bkl_lvl==0)			
		printk("%s:bl_level=%d enter1\n",  __func__,bkl_lvl);
	else
	{
		if(pre_bkl_lvl==0)
		printk("%s:bl_level=%d enter1\n",  __func__,bkl_lvl);
	...
	/*TYRD wuxh modify begin for bug CQWB00000946 on 20121129*/
	//if (!mfd->panel_power_on ) {
	//if ((!mfd->panel_power_on) ||msm_fb_supend) {
	/*TYRD wuxh modify end for bug CQWB00000946 on 20121129*/
	...
	if ((pdata) && (pdata->set_backlight)) {
		...
		//printk("%s:mfd->bl_level=%d enter2\n",  __func__,mfd->bl_level);
			    ...
			    boolean op_enable)
{
	...
	printk("msm_fb_blank_sub blank_mode =%d\r\n",blank_mode);
	switch (blank_mode) {
...
*/
	...
	default:
		...
		#ifdef TYQ_HX8392_MIPI_HD_SUPPORT 
