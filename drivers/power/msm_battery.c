...
//TYDRV: xiangdong add for 4350V battery
#if defined(TYQ_BATTERY_4350V_SUPPORT)
...
struct msm_battery_info {
	...
	u32 psy_status;
	u32 psy_health;
	  /*tydrv qupw add for check batt id */
#ifdef CONFIG_BQ2202A_BATT_ID
    char check_manufacture[10];
#endif
};
...
//TYDRV:xiangdong add for CQWB00000157 START
#if defined(TYQ_BATTERY_SUPPORT)
//TYDRV: xiangdong add for 4350V battery
#if defined(TYQ_BATTERY_4350V_SUPPORT)
#define	TYQ_BATTERY_FULL_DESIGN	1500  //1500 ma/h
#else

#if defined(TYQ_TBW5937_SUPPORT)
#define	TYQ_BATTERY_FULL_DESIGN	2800  //2800 ma/h
#elif (defined(TYQ_TBW5935_SUPPORT)||defined(TYQ_TBW5933_SUPPORT))
#define	TYQ_BATTERY_FULL_DESIGN	2000  //2000 ma/h
#else
#define	TYQ_BATTERY_FULL_DESIGN	1600  //1600 ma/h
#endif

#endif
#endif
//TYDRV:xiangdong add for CQWB00000157 END
/*tydrv qupw add for check batt id */
#ifdef CONFIG_BQ2202A_BATT_ID
extern u8 bq2202a_report_batt_status(void);
#endif
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		val->intval = TYQ_BATTERY_FULL_DESIGN;
		break;
	#endif
	//TYDRV:xiangdong add for CQWB00000157 END
	...
	case POWER_SUPPLY_PROP_MANUFACTURER:
		val->strval = msm_battery_info.check_manufacture;
		break;
#endif
...
//tydrv:xiangdong modify for correct battery temperature start
#if defined(TYQ_BATTERY_SUPPORT)
#define	TYQ_BATTEMP_THRESHOLD	200  //20*C
#define	TYQ_BATTEMP_SAMPLE_COUNT	3
static s32 tyq_battery_temp_buffer[3][2] = {
		{0,0},
		{0,0},
		{0,0}
		};
static s32 tyq_battery_temp_counter = 0;

static s32 tyq_vbatt_read_temperature(void)
{
	return msm_battery_info.battery_temp;
}
static s32 calculate_vbatt_temperature(s32 temp, s32 prevtemp)
{	
	if(
		(temp > (prevtemp + TYQ_BATTEMP_THRESHOLD)) &&
		(tyq_battery_temp_counter < TYQ_BATTEMP_SAMPLE_COUNT)
		){		
		tyq_battery_temp_buffer[tyq_battery_temp_counter][0] = temp;
		tyq_battery_temp_buffer[tyq_battery_temp_counter][1] = temp - prevtemp;
		tyq_battery_temp_counter++;
		temp = prevtemp;		
	}else if(		
		(temp > (prevtemp + TYQ_BATTEMP_THRESHOLD)) &&
		(tyq_battery_temp_counter = TYQ_BATTEMP_SAMPLE_COUNT)
	){
		tyq_battery_temp_counter = 0;
		temp = prevtemp + (tyq_battery_temp_buffer[0][1] + tyq_battery_temp_buffer[1][1] + tyq_battery_temp_buffer[2][1]) / TYQ_BATTEMP_SAMPLE_COUNT;
	}else if(
		(temp < (prevtemp + TYQ_BATTEMP_THRESHOLD))||
		(temp = (prevtemp + TYQ_BATTEMP_THRESHOLD))||
		(tyq_battery_temp_counter > TYQ_BATTEMP_SAMPLE_COUNT)
...
#endif
//tydrv:xiangdong modify for correct battery temperature end
...
void msm_battery_update_psy_status(void)
{
	...
	//tydrv:xiangdong modify for correct battery temperature start
	#if defined(TYQ_BATTERY_SUPPORT)	
	s32 tyq_prev_battery_temp = tyq_vbatt_read_temperature();
	#endif
	//tydrv:xiangdong modify for correct battery temperature end
	...
	//tydrv:xiangdong modify for correct battery temperature start
	#if defined(TYQ_BATTERY_SUPPORT)
	//tydrv:xiangdong modify for disable battemp detect
	#if defined(TYQ_BATTERY_TEMP_NOT_SUPPORT)
	tyq_prev_battery_temp = 200;
	battery_temp = 200;
	battery_temp = calculate_vbatt_temperature(200,200);	
	#else
	//battery_temp = (battery_temp > (tyq_prev_battery_temp + TYQ_BATTEMP_THRESHOLD)) ? tyq_prev_battery_temp : battery_temp;
	battery_temp = calculate_vbatt_temperature(battery_temp,tyq_prev_battery_temp);	
	#endif
	#endif
	//tydrv:xiangdong modify for correct battery temperature end
...
/*  tydrv qupw add for batt id check */
#ifdef CONFIG_BQ2202A_BATT_ID
if (bq2202a_report_batt_status())
	 strcpy(msm_battery_info.check_manufacture,"legal");
else
     strcpy(msm_battery_info.check_manufacture,"illegal");
#endif
