...
/*TYRD wuxh add for broadcom wlan  on 2012-05-22*/
#if 1
//#include <mach/ty_gpio.h>
...
static void gpio_wlan_config(void)
{
	if (machine_is_msm7627a_qrd1() || machine_is_msm7627a_evb()
		...
		/*TYDRV:chenjp 2012.12.26 modify for QCT support*/
		...
		if (on) {
			...
			if (rc) {
				pr_err("%s:%s vreg disable failed %d\n",
					__func__,
					vreg_info[index].vreg_id, rc);
				//xiangdong merge from QRD Workaround
				//goto reg_disable;
			}
		}
	}
...
#endif
/*TYRD wuxh add for broadcom wifi config on 20120423*/
#ifdef TYQ_CONFIG_BRCM_MODULE


#define WIFI_INT		20
#define WIFI_REG_ON		38
...
#define WLAN_SECTION_SIZE_0	(11 * 1024)
#define WLAN_SECTION_SIZE_1	(11 * 1024)
#define WLAN_SECTION_SIZE_2	(32 * 1024)
#define WLAN_SECTION_SIZE_3	(STATIC_BUF_SIZE+STATIC_BUF_TOTAL_LEN)
...
void *msm7x27_wifi_mem_prealloc(int section, unsigned long size)
{
...
EXPORT_SYMBOL(msm7x27_wifi_mem_prealloc);

int msm7x27_init_wifi_mem(void)
{
	int i;
...
static void power_on_wlan(void)
{
...
static void power_off_wlan(void)
{
...
void  wlan_mmc_init(void)
{
	int rc = 0;

	rc= gpio_tlmm_config(GPIO_CFG(WIFI_REG_ON, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
	                    GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc)
		printk(KERN_ERR "%s: Failed to configure wifi_pd %d\n",
	        __func__, rc);

	if (gpio_request(WIFI_REG_ON, "wifi_reg_on"))
		pr_err("wlan_mmc_init: failed to request gpio wifi_bt_pd\n");
	gpio_set_value(WIFI_REG_ON, 0);

	msm7x27_init_wifi_mem();

}


EXPORT_SYMBOL(wlan_mmc_init);
#endif
