...
/*TY wuxh modify for ty old 8x25 PROJECT Base design on 20121206*/
#ifdef TYQ_QCT_SUPPORT
#define FT5X06_IRQ_GPIO_QPR_SKUD_PRIM	40
#else
			     ...
			     char *buf)
{
	if (machine_is_msm8625q_skud() || machine_is_msm8625q_evbd()) {
		...
		return snprintf(buf, 200,
			__stringify(EV_KEY) ":" __stringify(KEY_MENU) ":127:1010:100:80"
		   ":" __stringify(EV_KEY) ":" __stringify(KEY_HOME) ":240:1010:100:80"
		...
		return snprintf(buf, 200,
		   ...
		   ":" __stringify(EV_KEY) ":" __stringify(KEY_HOME) ":350:1343:100:80"
...
static void __init ft5x06_touchpad_setup(void)
{
	...
	if (machine_is_msm8625q_skud()) {
		if (cpu_is_msm8625()) {
			...
			#elif defined (TYQ_TBW5931_FWVGA_SUPPORT)
			ft5x06_platformdata.x_max = 480;
			...
			ft5x06_platformdata.y_max = 854;
			#elif defined (TYQ_TP_THIRD_MENU_720x1280)
			ft5x06_platformdata.x_max = 720;
			ft5x06_platformdata.y_max = 1280;
			#else
			ft5x06_platformdata.x_max = 540;
			ft5x06_platformdata.y_max = 960;
			#endif
	...
	#if defined(TYQ_TBW5932_SUPPORT)
	rc = gpio_tlmm_config(GPIO_CFG(113, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
	if (rc)
		pr_err("%s: gpio_tlmm_config for TP_ID(GPIO_113) failed\n",
			__func__);
	#endif
...
#endif
#ifdef CONFIG_TY_TOUCHSCREEN_MSTAR
#define TY_TP_INT 40
#define TY_TP_RST 26

static struct i2c_board_info i2c_1_devices[] = {
//#if defined(CONFIG_TY_TOUCHSCREEN_MSTAR)
		{
		I2C_BOARD_INFO("ms-msg21xx", 0x26),
		.irq = MSM_GPIO_TO_INT(TY_TP_INT),
		},		
//#endif
};
static void __init msg21xx_touchpad_setup(void)
{
	int rc;
	...
	if (rc)
		pr_err("%s: gpio_tlmm_config for TP_ID(GPIO_113) failed\n",
			__func__);
	#endif
	rc = gpio_tlmm_config(GPIO_CFG(TY_TP_INT, 0,
			GPIO_CFG_INPUT, GPIO_CFG_PULL_UP,
			GPIO_CFG_8MA), GPIO_CFG_ENABLE);
	if (rc)
		pr_err("%s: gpio_tlmm_config for %d failed\n",
	...
	rc = gpio_tlmm_config(GPIO_CFG(TY_TP_RST, 0,
			GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN,
			GPIO_CFG_8MA), GPIO_CFG_ENABLE);
	if (rc)
		pr_err("%s: gpio_tlmm_config for %d failed\n",
