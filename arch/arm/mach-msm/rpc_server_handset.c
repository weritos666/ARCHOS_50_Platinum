...
/*niuli add only support headset 20120124*/
#if defined(TYQ_FILTER_HEADSET_SUPPORT)
static struct hrtimer headset_timer;
static bool tyq_rec_media_key; /* ready for receive media key */
static enum hrtimer_restart headset_timer_func(struct hrtimer *data)
{
	printk("headset_timer_func\n");
	tyq_rec_media_key=true;
...
/*niuli add only support headset 20120124*/
#if defined(TYQ_FILTER_HEADSET_SUPPORT)
		if(tyq_rec_media_key==true && key == KEY_MEDIA)
		{
			printk("tyq_rec_media_key=%d\n",tyq_rec_media_key);
			tyq_rec_media_key = false;
			return;
		}
		printk("key media/end %d\n",key);
#endif
...
/*niuli add only support headset 20120124*/
#if defined(TYQ_FILTER_HEADSET_SUPPORT)
		hrtimer_cancel(&headset_timer);
		tyq_rec_media_key=false;
		
		if(key_code != HS_REL_K)
		{
			hrtimer_start(&headset_timer, ktime_set(2,0), HRTIMER_MODE_REL);
		}
#endif
...
static int __devinit hs_probe(struct platform_device *pdev)
{
	...
	/*niuli add only support headset 20120124*/
	#if defined(TYQ_FILTER_HEADSET_SUPPORT)
	hrtimer_init(&headset_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	headset_timer.function = headset_timer_func;
	tyq_rec_media_key=false;	
	#endif
